...
load_env_variables() {
    ...
    if [ -f "$ENV_CANONICAL" ]; then
        _log_debug "Loading environment variables from $ENV_CANONICAL"
        ...
        while IFS='=' read -r key value; do
            ...
            if [[ "$key" =~ ^[a-zA-Z_][a-zA-Z0-9_]*$ ]]; then
                ...
                export "$key=$value"
                _log_debug "Loaded: $key"
            else
                 _log_debug "Skipped invalid key: $key"
            fi
        done < <(grep -vE '^\s*#' "$ENV_CANONICAL" | grep '=') # Filter comments/blanks, ensure '=' exists
    else
        _log_debug "Warning: Environment file $ENV_CANONICAL not found."
    fi
    ...
    _log_debug "Finished loading environment variables."
}
...
update_env_variable() {
    ...
    if [[ -z "$var_name" ]]; then
        _log_debug "Error: update_env_variable called with empty variable name."
        return 1
    fi
    if [[ ! "$var_name" =~ ^[a-zA-Z_][a-zA-Z0-9_]*$ ]]; then
        _log_debug "Error: update_env_variable called with invalid variable name: $var_name"
        return 1
    fi

    _log_debug "Updating $var_name to '$new_value' in $ENV_CANONICAL"

    # Ensure the .env file exists, create if not
    if [ ! -f "$ENV_CANONICAL" ]; then
        _log_debug "Creating $ENV_CANONICAL as it does not exist."
        touch "$ENV_CANONICAL" || { _log_debug "Error: Failed to create $ENV_CANONICAL"; return 1; }
    ...
    local temp_file
    temp_file=$(mktemp) || { _log_debug "Error: Failed to create temp file for update."; return 1; }
    ...
    if [[ $found -eq 0 ]]; then
        _log_debug "Variable $var_name not found, appending to $ENV_CANONICAL"
    ...
    if [[ $? -ne 0 ]]; then
        _log_debug "Error: Failed to move temp file to $ENV_CANONICAL"
        return 1
    fi

    _log_debug "Successfully updated $var_name in $ENV_CANONICAL"
    return 0
}
...
update_available_images() {
    ...
    _log_debug "Updating AVAILABLE_IMAGES with: $images_str"
...
update_default_run_options() {
    ...
    if [[ $success -eq 0 ]]; then
        _log_debug "Successfully updated default run options in .env"
        return 0
    else
        _log_debug "Error updating one or more default run options in .env"
        return 1
    fi
}
...
# └── ...                        <- Other project files
#
# Description: .env file helpers. Added functions for getting/setting specific vars, handling AVAILABLE_IMAGES array, and run options.
